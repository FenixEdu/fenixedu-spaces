  	...
  	<table class="table">
   	...
   	<c:if test="${not empty comments}">
   		<c:forEach var="comment" items="${comments}">
   			<div class="panel panel-default">
  				<div class="panel-heading">${comment.owner.presentationName} - ${comment.instant.toDate()} </div>
   	...
   	<form class="form" role="form" action="${commentUrl}" method="post">
  		<div class="form-group">
   		  <label class="sr-only" for="descriptionInput"><spring:message code="occupations.requests.add.comment" text="Add comment"/></label>
    	  <textarea rows="10" cols="60" class="form-control" id="descriptionInput" name="description" required="required"></textarea>
  		</div>
  		<div class="form-group">
  			<label class="radio-inline">
    			<input type="radio" name="state" id="stateNEW" value="NEW">
