...
public class PunctualRoomsOccupationComment extends PunctualRoomsOccupationComment_Base {
    ...
    public PunctualRoomsOccupationComment(PunctualRoomsOccupationRequest request, MultiLanguageString subject,
            MultiLanguageString description, User owner, DateTime instant) {
...
//        check(this, ResourceAllocationRolePredicates.checkPermissionsToManagePunctualRoomsOccupationComments);
        if (!getRequest().getCurrentState().equals(RequestState.NEW)) {
            throw new SpaceDomainException("error.PunctualRoomsOccupationRequest.impossible.edit");
        }
        setSubject(subject);
        setDescription(description);
    }
    ...
    public void setInstant(DateTime instant) {
        if (instant == null) {
            throw new SpaceDomainException("error.PunctualRoomsOccupationComment.empty.instant");
        }
    ...
    public void setDescription(MultiLanguageString description) {
        if (description == null || description.isEmpty()) {
            throw new SpaceDomainException("error.PunctualRoomsOccupationComment.empty.description");
        }
    ...
    public void setSubject(MultiLanguageString subject) {
        if (subject == null || subject.isEmpty()) {
            throw new SpaceDomainException("error.PunctualRoomsOccupationComment.empty.subject");
        }
    ...
    public void setRequest(PunctualRoomsOccupationRequest request) {
        if (request == null) {
            throw new SpaceDomainException("error.PunctualRoomsOccupationComment.empty.request");
        }
    ...
    public void setOwner(User owner) {
        if (owner == null) {
            throw new SpaceDomainException("error.PunctualRoomsOccupationComment.empty.owner");
        }
        super.setOwner(owner);
    }

    private void checkIfCommentAlreadyExists(User owner, MultiLanguageString subject, MultiLanguageString description) {
        Collection<PunctualRoomsOccupationComment> comments = owner.getPunctualRoomsOccupationCommentsSet();
        for (PunctualRoomsOccupationComment comment : comments) {
            if (comment.getDescription().compareTo(description) == 0) {
                throw new SpaceDomainException("error.PunctualRoomsOccupationComment.comment.already.exists");
            }
        }
    }
