...
//        check(this, SpacePredicates.checkPermissionsToManageSpace);
        if (!canBeDeleted()) {
            throw new SpaceDomainException("error.room.cannot.be.deleted");
        }
        super.delete();
    }

    private boolean canBeDeleted() {
        //TODO: try to write with tx abort if failed.
//        return !hasAnyAssociatedSummaries() && !hasAnyWrittenEvaluationEnrolments() && !hasAnyAssociatedInquiriesRooms();
    ...
    public static abstract class RoomFactory implements Serializable, SpaceFactoryExecutor {
