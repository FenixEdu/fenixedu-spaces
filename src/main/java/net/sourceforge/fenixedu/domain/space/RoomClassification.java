...
//        check(this, SpacePredicates.checkIfLoggedPersonHasPermissionsToManageRoomClassifications);
        ...
        if (existingRoomClassification != null && existingRoomClassification != this) {
            throw new SpaceDomainException("error.room.classification.with.same.code.exists", new String[] { code.toString() });
        }
        if (code == null) {
            throw new SpaceDomainException("error.room.classification.cannot.have.null.code");
        }
        if (name == null || name.getAllContents().isEmpty()) {
            throw new SpaceDomainException("error.room.classification.cannot.have.null.name");
        }
...
//        check(this, SpacePredicates.checkIfLoggedPersonHasPermissionsToManageRoomClassifications);
        if (getChildRoomClassificationsSet().isEmpty()) {
            ...
            throw new SpaceDomainException("error.cannot.delete.room.classification.with.existing.child.classifications");
        }
    }
    ...
    public static SortedSet<RoomClassification> sortByRoomClassificationAndCode(
        ...
        final TreeSet<RoomClassification> newTreeSet = Sets.newTreeSet(COMPARATORY_BY_PARENT_ROOM_CLASSIFICATION_AND_CODE);
        newTreeSet.addAll(roomClassifications);
        return newTreeSet;
    }
    ...
    public static abstract class RoomClassificationFactory implements Serializable, SpaceFactoryExecutor {
        private String code;
            ...
            if (getCode() != null) {
                ...
                if (index > 0) {
                    ...
                    if (roomClassification == null) {
                        throw new SpaceDomainException("error.unexisting.room.classification");
                    } else {
                        return roomClassification;
                    }
                }
        ...
        public Integer getChildCode() {
            if (getCode() == null || StringUtils.isEmpty(getCode().trim())) {
                ...
                throw new SpaceDomainException("error.roomClassification.code.isnt.a.number");
            }
